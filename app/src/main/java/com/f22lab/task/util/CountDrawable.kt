...
class CountDrawable(context: Context) : Drawable() {
    ...
    override fun draw(canvas: Canvas) {
        ...
        if (mCount.length <= 2) {
            // Draw badge circle.
            canvas.drawCircle(centerX, centerY, (radius + 10.5).toInt().toFloat(), mBadgePaint)
        } else {
            canvas.drawCircle(centerX, centerY, (radius + 10.5).toInt().toFloat(), mBadgePaint)
        }
   ...
   companion object {
       fun updateCount(menuItem: MenuItem, context: Context, count: String) {
